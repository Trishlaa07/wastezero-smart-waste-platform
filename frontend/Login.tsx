...
import { Trash2, Lock, Mail, KeyRound, ArrowLeft } from "lucide-react";
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (otp === "123456") { // Mock OTP for demonstration
        ...
        setErrors({ otp: "Invalid OTP. Use 123456 for testing." });
      }
    }, 1500);
  };

  return (
    <div className="flex min-h-screen bg-brand-pale overflow-hidden font-sans text-brand-dark">
              ...
              {step === 'login' ? (
                ...
                <>
                  <button
                    ...
                    className="flex items-center gap-1 text-sm text-brand-medium hover:text-brand-dark mb-4 transition-colors"
                  >
                    <ArrowLeft className="w-4 h-4" /> Back to login
                  </button>
                  <h2 className="text-3xl font-bold text-gray-900 mb-2">Verify OTP</h2>
                  <p className="text-gray-500 text-sm">We've sent a 6-digit code to <span className="font-semibold text-brand-dark">{formData.email}</span></p>
                </>
              )}
            </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        placeholder="000000"
                        maxLength={6}
                        className="pl-10 text-center tracking-[0.5em] text-lg font-bold border-gray-200 focus:border-brand-medium focus:ring-brand-medium/20 transition-all rounded-lg"
                        value={otp}
                ...
                <div className="text-center mt-6">
                  <p className="text-gray-500 text-sm">
                    ...
                    <button
                      ...
                      className="font-medium text-brand-medium hover:text-brand-dark transition-colors underline decoration-2 decoration-transparent hover:decoration-brand-medium animation-all"
                    >
                      Resend OTP
                    </button>
                  </p>
